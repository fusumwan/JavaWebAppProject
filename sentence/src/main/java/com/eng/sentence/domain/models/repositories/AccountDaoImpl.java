...
public class AccountDaoImpl implements AccountDao {
	...
	String password_01
		...
		String hql="FROM Account WHERE username=:username_00 AND password=:password_01";
		List<Account> accounts = null;
		if(!hql.isEmpty()) {
			Session currentSession = sessionFactory.getCurrentSession();
			Query<Account> theQuery = currentSession.createQuery(hql, Account.class);
			theQuery.setParameter("username_00", username_00);
			theQuery.setParameter("password_01", password_01);
			accounts = theQuery.getResultList();
		}
		return accounts;
	}
	...
	String username_00
		...
		String hql="FROM Account WHERE username=:username_00";
		List<Account> accounts = null;
		if(!hql.isEmpty()) {
			Session currentSession = sessionFactory.getCurrentSession();
			Query<Account> theQuery = currentSession.createQuery(hql, Account.class);
